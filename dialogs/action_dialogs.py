...
class MigrateProcessDialog(_BaseActionDialog):
    """Collects target PID for migration."""

    def __init__(self, parent=None):
        super().__init__("Migrate Process", parent)
        self.pid_spin = QSpinBox()
        self.pid_spin.setRange(0, 999999)
        self.pid_spin.setValue(0)
        self._layout.addRow("Target PID:", self.pid_spin)
...
class HollowProcessDialog(_BaseActionDialog):
    """Collects target process path and BOF for process hollowing."""

    def __init__(self, parent=None):
        super().__init__("Hollow Process", parent)
        self.proc_path_input = QLineEdit()
        self.proc_path_input.setPlaceholderText("C:\\Windows\\System32\\svchost.exe")
        self._layout.addRow("Target Process:", self.proc_path_input)

        # Payload file row with browse button
        bof_row = QHBoxLayout()
        self.bof_path_input = QLineEdit()
        self.bof_path_input.setPlaceholderText("/path/to/payload.bin")
        self.bof_browse_btn = QPushButton("Browse...")
        self.bof_browse_btn.clicked.connect(self._browse_payload)
        bof_row.addWidget(self.bof_path_input)
        bof_row.addWidget(self.bof_browse_btn)
    ...
    def _browse_payload(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Payload File", "",
            "Payload Files (*.o *.obj *.bin *.raw *.exe *.dll);;All Files (*)"
        )
        if path:
            self.bof_path_input.setText(path)

    def get_process_path(self) -> str:
        return self.proc_path_input.text()
